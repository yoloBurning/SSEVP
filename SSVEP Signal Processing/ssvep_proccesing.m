...
weight_factor = 0.8; % 0.8 should be the optimal
% weight factor to extrach the feature of the signal
% combined with 2 order harmonics frequency
for trial = 1:size(PSD,3)
    for ch = 1:size(PSD,1) 

        FEAT.low(trial,ch)  = weight_factor*PSD(ch,f==7.5,trial)+...
            (1-weight_factor)*weight_factor*PSD(ch,f==15,trial);
        
        FEAT.high(trial,ch) = weight_factor*PSD(ch,f==12.25,trial)+...
            (1-weight_factor)*PSD(ch,f==37,trial);
...
%% Check which is the optimal number of features we should use
% Bi.Wang's code is here
FEAT.allFeatures = cat(2,FEAT.low,FEAT.high,FEAT.ref);
FEAT.labels = lab';
% FEAT.allFeatures(FEAT.labels==0,:)=[];
% FEAT.labels(FEAT.labels==0)=[];
FEAT.labels(FEAT.labels==1)=-1;
FEAT.labels(FEAT.labels==2)=-1;
FEAT.labels(FEAT.labels==0)=1;
% change label 2 into label -1
% -1 = 7.5Hz
% 1  = 12Hz
[FEAT.d_all FEAT.rank_all] = fisherrank(FEAT.allFeatures,FEAT.labels); % compute fisher score of all features
% Visualize the data in between steps for plausibilty checks
% if visualize
%     figure(4)
%     subplot(1,2,1)
%     plot(FEAT.d_all(FEAT.rank_all)) % plot fisher scores of the ranked features
%     xlabel('number of best features')
%     ylabel('fisher score')
%     grid on
    
%     % visualize 2 best features in scatter plot
%     subplot(1,2,2)

    gscatter(FEAT.allFeatures(:,FEAT.rank_all(1)),FEAT.allFeatures(:,FEAT.rank_all(2)),FEAT.labels)
    xlabel('first best feature')
    ylabel('second best feature')
%     grid on   
% end  

% This step is just for further investigation of the features
[FEAT.d_low FEAT.rank_low] = fisherrank(FEAT.low,FEAT.labels); % compute fisher score for alpha only
[FEAT.d_high FEAT.rank_high] = fisherrank(FEAT.high,FEAT.labels); % compute fisher score for beta only

z = zeros(100,6);
for mm = 1:100
% calculate the error rate corresponding to the different features we
% choose by using cross validation to check which is the best choice for us
% to choose the first n best features to train our model
for i = 1:6
nFolds = 8;
...
for j = 1:i
a = [a FEAT.allFeatures(:,FEAT.rank_all(j))];
end
FEAT.allFeature = a;
for fold = 1:nFolds
    test = (indices == fold); train = ~test; 
    model.class = trainShrinkLDA(FEAT.allFeature(train,:),FEAT.labels(train),0.1,'');
    y = predictShrinkLDA(model.class,FEAT.allFeature(test,:));
    MCR(fold) = sum(FEAT.labels(test) ~= y')/length(FEAT.labels(test)); % Ref 
end
avgMCR = mean(MCR);    
disp(avgMCR)
z(mm,i) = avgMCR;
...
% Build 1 vs 0&2
FEAT.label1 = Label;
FEAT.allFeature1 = FEAT.allFeatures;
FEAT.allFeature1(FEAT.label1==0,:) = [];
FEAT.label1(FEAT.label1==0)=[];
FEAT.label1(FEAT.label1==2)=-1;
FEAT.label1(FEAT.label1==1)=1;
[FEAT.d_all_1 FEAT.rank_all_1] = fisherrank(FEAT.allFeature1,FEAT.label1);
% change label 2 into label -1
% -1 = 7.5Hz
% 1  = 12Hz
